...
def log_water():
    ...
    return jsonify(
        {"habit": "water", "value": value, "unit": "ml", "timestamp": now}
    )
...
def water_summary():
    ...
    """).fetchone()
    avg_water = (
        int(avg_result[0]) if avg_result and avg_result[0] is not None else 0
    )
...
def toggle_sleep():
    ...
    session = db.execute(
        "SELECT start_time FROM active_sessions WHERE habit = 'sleep'"
    ).fetchone()
    now = datetime.now()
...
def sleep_summary():
    ...
    """).fetchone()
    avg_hours = (
        avg_result[0] if avg_result and avg_result[0] is not None else 0.0
    )
...
def view_logs():
    db = get_db()
    rows = db.execute(
        "SELECT id, habit, timestamp FROM logs ORDER BY id DESC"
    ).fetchall()
    db.close()
    return jsonify(
        [{"id": r[0], "habit": r[1], "timestamp": r[2]} for r in rows]
    )
...
def view_page():
    banana = get_db()
    rows = banana.execute(
        "SELECT habit, timestamp FROM logs ORDER BY id DESC"
    ).fetchall()
    banana.close()
